...
async function runTests() {
    await (0, data_js_1.loadChampionData)();
    console.log("=== SCENARIO 1: Top vs 3 AD (Expect Malphite top 3) ===");
    const rec1 = await (0, recommendation_js_1.getRecommendation)({
        role: 'Top',
        side: 'blue',
        allies: [],
    ...
    const rec2 = await (0, recommendation_js_1.getRecommendation)({
        role: 'Mid',
        side: 'blue',
        allies: [],
    ...
    const rec3 = await (0, recommendation_js_1.getRecommendation)({
        role: 'ADC',
        side: 'blue',
        allies: ['Leona'],
        enemies: [],
    ...
    const rec4 = await (0, recommendation_js_1.getRecommendation)({
        role: 'Mid',
        side: 'blue',
        allies: [],
        enemies: [],
...
function printTop(rec, expectChamp) {
    const all = [rec.best, ...rec.alternatives];
    let found = false;
    all.forEach((c, i) => {
        if (c.name === expectChamp)
            found = true;
    ...
    if (!found) {
        ...
        const c = rec.allScored?.find((x) => x.name === expectChamp);
        if (c) {
