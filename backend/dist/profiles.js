...
exports.champArchetypes = {
    ...
    Jinx: { 'provides': ['Damage', 'Scaling', 'Zone'], 'wants': ['Peel', 'Frontline', 'CC', 'Engage'], 'style': ['Scaling', 'Teamfight'], 'traits': ['Hypercarry', 'Immobile'], 'safeBlind': true, 'counterReliant': false, 'hardWeakInto': ['Dive', 'Pick'], 'peakPhase': 'late' },
...
function buildAutoProfile(champion, role) {
    ...
    const profile = {
        ...
        hardWeakInto: [],
        weaknesses: [],
        metaScore: (0, metaService_js_1.getChampionMetaScore)(champion.id, role, 'emerald_plus', data_js_1.latestPatch),
        style: [],
        traits: [],
        flexRoles: [],
        flexAmbiguity: 0,
    ...
    const possibleRoles = Object.keys(rolePool_js_1.rolePool).filter(r => rolePool_js_1.rolePool[r].includes(champion.id));
    ...
    if (tags.includes('Marksman') || tags.includes('Mage'))
        profile.wants.push('Frontline');
    if (tags.includes('Marksman'))
        profile.wants.push('Peel');
...
function getProfile(champion, role) {
    const base = buildAutoProfile(champion, role);
    const override = exports.champArchetypes[champion.id] || {};
    return {
        ...base,
        ...override,
